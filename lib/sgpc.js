...
const fetchLatest = async () => {
  ...
  const mukhvaakRaw = document.querySelectorAll( '.hukamnama-text' )[ 0 ]
    .textContent
    .replace( /[\r\n\t]/g, '' )
    .replace( /\s+/g, ' ' )
  ...
  // Get page number by finding element that matches the pattern
  const customDateElements = Array.from( document.querySelectorAll( '.customDate' ) )
  let page = null
  const pagePattern = /\((ਅੰਗ|Ang|Page)\s*:?\s*(\d+|[\u0A66-\u0A6F]+)\)/u

  const pageElement = customDateElements.find( element => {
    const text = element.textContent
    return pagePattern.test( text )
  } )
  if ( pageElement ) {
    const pageMatch = pagePattern.exec( pageElement.textContent )[ 2 ]
    page = isGurmukhi( pageMatch ) ? toAscii( pageMatch ) : pageMatch
    page = parseInt( page, 10 )
  }
  if ( !page ) {
    throw new Error( 'SGPC - Could not extract page number from available elements' )
  }

  // Get date by finding element that matches the date pattern
  let dateRaw = null
  const datePattern = /(\d{1,2}-\d{1,2}-\d{4})/

  const dateElement = customDateElements.find( element => {
    const text = element.textContent.trim()
    return datePattern.test( text )
  } )
  if ( dateElement ) {
    dateRaw = dateElement.textContent.trim()
  }
  if ( !dateRaw ) {
    throw new Error( 'SGPC - Could not extract date from available elements' )
  }
